    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_PD_MODE_V_DATA_0: begin
                    rdata <= int_pd_mode_V[0:0];
                end
                ...
                ADDR_CONV_W_DATA_0: begin
                    rdata <= int_conv_w[31:0];
                end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_PD_MODE_V_DATA_0)
            int_pd_mode_V[0:0] <= (WDATA[31:0] & wmask) | (int_pd_mode_V[0:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_CONV_W_DATA_0)
            int_conv_w[31:0] <= (WDATA[31:0] & wmask) | (int_conv_w[31:0] & ~wmask);
    end
end
