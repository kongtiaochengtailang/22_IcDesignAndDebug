...
architecture behave of Conv_AXILiteS_s_axi is
    ...
    constant ADDR_PD_MODE_V_DATA_0   : INTEGER := 16#50#;
    constant ADDR_PD_MODE_V_CTRL     : INTEGER := 16#54#;
    ...
    constant ADDR_CONV_W_DATA_0      : INTEGER := 16#68#;
    constant ADDR_CONV_W_CTRL        : INTEGER := 16#6c#;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_PD_MODE_V_DATA_0 =>
                        rdata_data <= RESIZE(int_pd_mode_V(0 downto 0), 32);
                    ...
                    when ADDR_CONV_W_DATA_0 =>
                        rdata_data <= RESIZE(int_conv_w(31 downto 0), 32);
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_PD_MODE_V_DATA_0) then
                    int_pd_mode_V(0 downto 0) <= (UNSIGNED(WDATA(0 downto 0)) and wmask(0 downto 0)) or ((not wmask(0 downto 0)) and int_pd_mode_V(0 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_CONV_W_DATA_0) then
                    int_conv_w(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_conv_w(31 downto 0));
                end if;
            end if;
        end if;
    end process;
