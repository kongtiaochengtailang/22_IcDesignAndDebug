...
entity pooling_AXILiteS_s_axi is
generic (
...
end entity pooling_AXILiteS_s_axi;
...
architecture behave of pooling_AXILiteS_s_axi is
    ...
    constant ADDR_SC_V_DATA_0        : INTEGER := 16#38#;
    constant ADDR_SC_V_CTRL          : INTEGER := 16#3c#;
    ...
    constant ADDR_FEATURE_IN_DATA_0  : INTEGER := 16#58#;
    constant ADDR_FEATURE_IN_CTRL    : INTEGER := 16#5c#;
    constant ADDR_FEATURE_OUT_DATA_0 : INTEGER := 16#60#;
    constant ADDR_FEATURE_OUT_CTRL   : INTEGER := 16#64#;
    ...
    signal int_Sc_V            : UNSIGNED(7 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_SC_V_DATA_0 =>
                        rdata_data <= RESIZE(int_Sc_V(7 downto 0), 32);
...
-- ----------------------- Register logic ----------------
    ...
    Sc_V                 <= STD_LOGIC_VECTOR(int_Sc_V);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_SC_V_DATA_0) then
                    int_Sc_V(7 downto 0) <= (UNSIGNED(WDATA(7 downto 0)) and wmask(7 downto 0)) or ((not wmask(7 downto 0)) and int_Sc_V(7 downto 0));
                end if;
            end if;
        end if;
    end process;
