...
typedef struct {
    ...
    XPooling_uio_map maps[ MAX_UIO_MAPS ];
} XPooling_uio_info;

/***************** Variable Definitions **************************************/
static XPooling_uio_info uio_info;
...
static int uio_info_read_name(XPooling_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XPooling_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XPooling_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XPooling_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XPooling_Initialize(XPooling *InstancePtr, const char* InstanceName) {
	XPooling_uio_info *InfoPtr = &uio_info;
